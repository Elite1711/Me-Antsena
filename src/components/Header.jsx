    ...
    <header className="sticky top-0 z-50 border-b border-black/5 bg-white/90 backdrop-blur-xl dark:border-white/10 dark:bg-[#17002d]/90">
      <div className="container-app flex h-20 items-center gap-4">
        ...
        <nav className="hidden items-center gap-6 lg:flex" aria-label="Navigation principale">
          {[t("nav.home"), t("nav.products"), t("nav.categories"), t("nav.recommendations")].map((label) => {
            const to = label === t("nav.home") ? "/" : label === t("nav.products") ? "/products" : label === t("nav.categories") ? "/categories" : "/recommendations";
      ...
      {open && <div className="border-t border-black/5 px-4 pb-4 pt-3 dark:border-white/10 lg:hidden">
        ...
        <div className="grid grid-cols-2 gap-2">{["/","/products","/categories","/recommendations"].map((to,i)=><Link key={to} onClick={()=>setOpen(false)} to={to} className="rounded-xl bg-brand-50 px-4 py-3 text-sm font-semibold text-brand-700 dark:bg-brand-900/30 dark:text-brand-200">{[t("nav.home"), t("nav.products"), t("nav.categories"), t("nav.recommendations")][i]}</Link>)}</div>
      </div>}
    </header>
  );
}
