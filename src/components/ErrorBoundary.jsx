...
export default class ErrorBoundary extends Component {
  ...
  render() {
    ...
    return <div className="grid min-h-screen place-items-center bg-[#FFF1F1] p-6 text-center dark:bg-[#17002d]"><div className="card max-w-md p-6 text-left"><div className="text-center"><p className="text-5xl font-black text-brand-500">Oups !</p><h1 className="mt-3 text-2xl font-black">Une erreur est survenue</h1><p className="mt-2 text-sm muted">L’interface a rencontré un problème. Vos données locales n’ont pas été supprimées.</p></div>
      ...
      {this.state.showDetails && <pre className="mt-4 max-h-64 overflow-auto rounded border bg-slate-50 p-3 text-xs dark:bg-[#0b0520]">{String(this.state.error && (this.state.error.stack || this.state.error))}</pre>}
    </div></div>;
  }
}
