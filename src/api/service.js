...
async function fetchMl(path, { timeoutMs = 4000 } = {}) {
  ...
  try {
    ...
    return null; // ml-service indisponible : on laisse l'appelant retomber sur son repli
  } finally {
    clearTimeout(timeout);
  }
}

async function hydrateProductIds(ids, existingProducts) {
  const byId = new Map(existingProducts.map(p => [String(p.id), p]));
  const missing = ids.filter(id => !byId.has(String(id)));
  if (missing.length) {
    const { data } = await supabase.from('products').select(`${PRODUCT_SELECT},reviews(rating)`).in('id', missing);
    for (const p of data || []) byId.set(String(p.id), { ...mapProduct(p), rating: p.reviews?.length ? p.reviews.reduce((a,r)=>a+Number(r.rating),0)/p.reviews.length : 0, reviews: p.reviews?.length || 0 });
  }
  return ids.map(id => byId.get(String(id))).filter(Boolean);
}

export async function getSimilarProducts(productId, allProducts = []) {
  const ml = await fetchMl(`/similar/${productId}?top_k=4`);
  if (ml?.items?.length) return hydrateProductIds(ml.items.map(i => i.product_id), allProducts);
  // Repli : produits de la même catégorie côté client
  const current = allProducts.find(p => String(p.id) === String(productId));
  return current ? allProducts.filter(x => x.category === current.category && String(x.id) !== String(productId)).slice(0, 4) : [];
}
...
export async function getRecommendations(){
  ...
  try {
    const { data, error } = await supabase
      ...
      .select('type,score,product:products(id,name,description,price,stock,category_id,images,tags,created_at,category:categories(id,name,icon),reviews(rating))')
      .order('score', { ascending: false });

    if (error) throw error;

    const grouped = { collaborative: [], content: [], hybrid: [] };
    const seen = new Set();

    for (const row of data || []) {
      const product = row?.product;
      if (!product || !row?.type || !product.id) continue;
      const key = `${row.type}:${product.id}`;
      if (seen.has(key)) continue;
      seen.add(key);

      const mapped = {
        ...mapProduct(product),
        rating: product.reviews?.length ? product.reviews.reduce((sum, review) => sum + Number(review.rating || 0), 0) / product.reviews.length : 0,
        reviews: product.reviews?.length || 0,
      };

      grouped[row.type]?.push(mapped);
    }
