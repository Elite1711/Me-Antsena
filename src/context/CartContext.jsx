...
export function CartProvider({ children }) {
  ...
  const addToCart = (product, quantity = 1) => {
    ...
    persist(next);
    logInteraction(product.id, "cart", requested);
    toast.success(existing && existing.quantity >= stock ? "Stock maximum déjà atteint" : "Produit ajouté au panier");
  };

  const removeFromCart = (id) => persist(items.filter(i => String(i.id) !== String(id)));
  ...
  const value = useMemo(() => ({ items, count, subtotal, shipping, total, addToCart, removeFromCart, updateQuantity, clearCart }), [items, count, subtotal, shipping, total]);
