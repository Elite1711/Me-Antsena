...
export default function AdminDashboard(){
  ...
  useEffect(()=>{
    getDashboardStats().then(setData).catch(()=>toast.error("Impossible de charger les statistiques"));
  ...
  const changeOrder=(id,status)=>{updateOrderStatus(id,status);toast.success("Statut de commande mis à jour")};
  ...
  return <>
    ...
    {tab==="products" && <Products products={filteredProducts} query={query} setQuery={setQuery} onDelete={deleteProduct} onEdit={p=>setModal({type:"product",product:p})} onAdd={()=>setModal({type:"product"})}/>} 
