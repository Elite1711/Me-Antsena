  ...
  return <div>
    <section className="container-app pt-5 sm:pt-8">
      <div className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-brand-900 via-brand-700 to-brand-500 px-6 py-10 text-white shadow-glow sm:px-10 lg:px-14 lg:py-14">
        ...
        <div className="relative max-w-2xl"><span className="mb-4 inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs font-bold backdrop-blur"><Sparkles size={14}/> {t("home.heroBadge")}</span><h1 className="text-4xl font-black tracking-tight sm:text-5xl">{t("home.heroTitle")}<br/><span className="text-orange-300">{t("home.heroTitleAccent")}</span></h1><p className="mt-4 max-w-xl text-sm leading-6 text-white/80 sm:text-base">{t("home.heroText")}</p><div className="mt-7 flex flex-wrap gap-3"><Link to="/products" className="btn-primary bg-white text-brand-700 hover:bg-brand-50">{t("home.discover")} <ArrowRight size={17}/></Link></div></div>
      </div>
    </section>
    ...
    <section className="container-app py-8"><SectionHeader title={t("home.recommendedForYou")} subtitle={t("home.basedOnCommunity")}/><div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">{data.recs?.hybrid.slice(0, 4).map(p=><ProductCard key={p.id} product={p} reason={t("home.recommendedForYou")}/>)}</div></section>
    <section className="container-app py-8"><SectionHeader title={t("home.recentConsults")} subtitle={t("home.contentFiltering")}/><div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">{data.recs?.content.slice(0, 4).map(p=><ProductCard key={p.id} product={p} reason={t("home.recentlyViewed")}/>)}</div></section>
