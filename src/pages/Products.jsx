...
export default function Products() {
  ...
  return <div className="container-app py-7">
    ...
    <div className="mb-5 flex flex-col gap-3 lg:flex-row">
      ...
      <div className={`${filtersOpen?"block":"hidden"} rounded-2xl border border-black/5 bg-white p-3 dark:border-white/10 dark:bg-[#241A10] lg:flex lg:items-center lg:gap-3 lg:border-0 lg:bg-transparent lg:p-0 lg:dark:bg-transparent`}>
    ...
    <div className="mb-5 flex items-center justify-between"><p className="text-sm font-semibold text-slate-500">{loading ? "Chargement..." : `${products.length} produit(s)`}</p><div className="hidden rounded-xl border border-black/5 bg-white p-1 dark:border-white/10 dark:bg-[#241A10] sm:flex"><button className={`rounded-lg p-2 ${view==="grid"?"bg-brand-50 text-brand-600":"text-slate-400"}`} onClick={()=>setView("grid")} aria-label="Vue grille"><Grid2X2 size={17}/></button><button className={`rounded-lg p-2 ${view==="list"?"bg-brand-50 text-brand-600":"text-slate-400"}`} onClick={()=>setView("list")} aria-label="Vue liste"><List size={17}/></button></div></div>
