...
export default function Checkout() {
  ...
  const confirm=async ()=>{ if(confirming) return; if(!address.trim()) { toast.error("Veuillez renseigner une adresse."); return; } setConfirming(true); try { const order=await addOrder({items,total,shipping,address:address.trim(),payment,delivery}); clearCart(); toast.success(`Commande ${order.id} confirmée !`); navigate("/profile?tab=orders", {replace:true}); } catch { setConfirming(false); toast.error("Impossible de confirmer la commande."); } };
