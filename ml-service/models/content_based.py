...
class ContentBasedModel:
    ...
    def add_product(self, product: dict) -> bool:
        """Indexe un produit à la volée avec le vectoriseur déjà entraîné (cold-start item).

        Permet de gérer un produit créé après le dernier entraînement, sans
        attendre un ré-entraînement complet du modèle : seul le vocabulaire
        appris au `fit()` initial est réutilisé (`vectorizer.transform`, pas
        `fit_transform`), donc un mot totalement absent du corpus d'origine
        sera ignoré — c'est la limite normale d'un TF-IDF non ré-entraîné.

        Retourne False si le modèle n'a pas encore été entraîné une première
        fois (il faut au moins un `fit()` pour disposer d'un vocabulaire).
        """
        ...
        if product_id in self.product_index:
            return True  # déjà indexé, rien à faire

        text = self._build_text(product)
        new_vector = self.vectorizer.transform([text])
        self.product_features = sp.vstack([self.product_features, new_vector])
        self.product_index[product_id] = len(self.product_ids)
